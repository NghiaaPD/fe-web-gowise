<script lang="ts">
    ...
    let {
        ...
        onsociallogin?: (
            event: CustomEvent<AuthFormEvents["socialLogin"]>,
        ) => void;
    } = $props();
        ...
        if (!isSignUp) {
            ...
            try {
                const res = await fetch(
                    "http://nghiapd.ddns.net:8081/auth/signin",
                    {
                        method: "POST",
                        headers: {
                ...
                if (!res.ok || !data.success) {
                    alert("Đăng nhập thất bại: " + (data?.message || data));
                    return;
                }
                ...
                setTimeout(() => {
                    ...
                    if (userRole === "admin" || userRole === "administrator") {
                        ...
                        // Fallback nếu không lấy được user_id
            ...
            try {
                const res = await fetch(
                    "http://nghiapd.ddns.net:8081/auth/signup",
                    {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
    ...
    function handleSocialLogin(provider: string) {
        onsociallogin?.(
            new CustomEvent("socialLogin", { detail: { provider } }),
        );
    }
</script>
...
>
    ...
    <form onsubmit={handleSubmit} class="space-y-3">
        ...
        {#if isSignUp}
            <div
                transition:slide={{ duration: 300, easing: quintOut, delay: 0 }}
            >
                <label
                    for="username"
